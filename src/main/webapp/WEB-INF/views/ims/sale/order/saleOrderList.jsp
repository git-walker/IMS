...
<script type="text/javascript">
    ...
    layui.use(['layer', 'form','table','laydate'], function() {
        ...
        var tableObj = table.render({
            title:'订单信息列表',
            elem: '#mainTable',
            url: '${ctx}/ims/sale/order/saleOrderList',
            method:'post',
        ...
        function initAdd() {
            $.get("${ctx}/ims/sale/order/initAddOrder",function (resp) {
                ...
                form.on("submit(addSaleOrderForm)",function(){
                    $("#addSaleOrderForm").ajaxSubmit({
                        url:'${ctx}/ims/sale/order/addOrder',
        ...
        function initOrderItemAdd(orderId){
            $.get("${ctx}/ims/sale/order/orderGoodsList?orderId="+orderId,function (resp) {
                        ...
                        layer.open({
                            ...
                            ,yes: function(index, layero){//按钮【按钮一】的回调
                                layer.confirm('确定完成录入并提交?', {
                                    ...
                                    layer.close(ind);
                                    $.get("${ctx}/ims/sale/order/haveAnyGoods?orderId="+orderId,function (data) {//订单是否为空
                                        if (data.count===0){
        ...
        function completeOrder(orderId) {
            $.get('${ctx}/ims/sale/order/completeOrder?orderId='+orderId,function (resp) {
                layer.msg(resp.msg);
            });
        }
        ...
        function initDelete() {
            ...
            layer.confirm('确定删除吗？', function(index){
                ...
                $.ajax({
                    url:"${ctx}/ims/sale/order/delOrder",
                    dataType:"json",
                    data:{
                        id:idArray.join(",")
                    },
        ...
        function initUpdate(){
            ...
            $.get("${ctx}/ims/sale/order/initEditOrder?id="+id,function (resp) {
                $("#addEditForm").html(resp);
                var index=layer.open({
                    type: 1,
                    area: '800px',
                ...
                form.on("submit(editSaleOrderForm)",function(){
                    $("#editSaleOrderForm").ajaxSubmit({
                        url:'${ctx}/ims/sale/order/updateOrder',
                        success:function(resp,textStatus,jqXHR,$form){
                            if (resp.success){
                                layer.close(index);
                                tableObj.reload();
        ...
        function initCommit() {
            ...
            layer.confirm('确定提交吗？', function(index){
                ...
                $.ajax({
                    url:"${ctx}/ims/sale/order/commitOrder",
                    dataType:"json",
                    data:{
                        id:idArray.join(",")
                    },

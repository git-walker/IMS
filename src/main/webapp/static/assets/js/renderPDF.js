;(function (global) {
	...
	// 'ledger': [1224, 792],
	// 'tabloid': [792, 1224]
	global.renderPDF = function (content, pdfName, format, onSuccess, direction) {
		...
		if (direction === 'l') {
			var temp = pdfFormat[format];
			pdfFormat[format][0] = temp[1];
			pdfFormat[format][1] = temp[0];
		}
		...
		function renderPages(imgData) {
			...
			var pdf = new jsPDF(direction, "pt", format);

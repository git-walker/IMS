	...
	navButtonAdd : function (elem, p) {
		...
		return this.each(function() {
			...
			if (findnav) {
				...
				//	$("tr",findnav).append(tbd);
				//}
				//修改自定义按钮位置   注：未修改findnav为false的情况      ---lijunjie
				if (typeof p.position === "number") {
					if (findnav.rows[0].cells.length === 0 || p.position <= 1 || p.position > findnav.rows[0].cells.length) {
						$("tr", findnav).append(tbd);
					} else {
						$("tr td:eq(" + (p.position - 1) + ")", findnav).before(tbd);
					}
